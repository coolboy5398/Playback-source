...
function overwriteRules(params) {
	// 自定义进程直连列表（仅桌面端支持，移动端无效）
	const customDirectProcesses = [
		// 在这里添加需要直连的进程名，每行一个
		// 'WeChat.exe',      // 微信
		// 'QQ.exe',          // QQ
		// 'DingTalk.exe',    // 钉钉
	...
	// 自定义进程代理列表（仅桌面端支持，移动端无效）
	const customProxyProcesses = [
		// 在这里继续添加需要走代理的进程名，每行一个
	...
	const rules = [
		...
		// 自动生成自定义进程规则（优先级最高）
		...customDirectProcesses.map(process => `PROCESS-NAME,${process},DIRECT`),
		...customProxyProcesses.map(process => `PROCESS-NAME,${process},${proxyName}`),
		// 自动生成自定义直连规则
		...customDirectDomains.map(domain => `DOMAIN-SUFFIX,${domain},DIRECT`),
	...
	const groups = [
		{
			...
			proxies: ['🎯手动选择', '🤖自动选择', '🔀负载均衡(散列)', '🔁负载均衡(轮询)', 'DIRECT']
		},
		{
			name: '🎯手动选择',
			type: 'select',
...
function overwriteDns(params) {
	// 引导 DNS (Bootstrap DNS)，必须是纯 IP 地址。用于解析其他 DNS 服务器和代理节点的域名。
	const bootstrapDns = ['223.5.5.5', '119.29.29.29', '120.53.53.53']
	const cnDnsList = ['https://223.5.5.5/dns-query', 'https://1.12.12.12/dns-query', 'https://doh.pub/dns-query']
	const trustDnsList = ['quic://dns.cooluc.com', 'https://1.0.0.1/dns-query', 'https://1.1.1.1/dns-query', 'https://dns.google/dns-query', 'https://cloudflare-dns.com/dns-query']
	...
	// 为直连域名生成 DNS 策略（自动使用国内 DNS）
	const customDirectDnsPolicy = {}
	customDirectDomains.forEach(domain => {
		customDirectDnsPolicy[domain] = cnDnsList
	})

	const dnsOptions = {
		enable: true,
		'prefer-h3': false, // 关闭DoH3优先，提升稳定性
		'default-nameserver': bootstrapDns, // 用于解析其他DNS服务器、和节点的域名, 必须为IP, 可为加密DNS。注意这个只用来解析节点和其他的dns，其他网络请求不归他管
		nameserver: trustDnsList, // 其他网络请求都归他管

		// 这个用于覆盖上面的 nameserver
		'nameserver-policy': {
			//[combinedUrls]: notionDns,
			...customDirectDnsPolicy, // 直连域名使用国内 DNS（linux.do, h-e.top 等）
